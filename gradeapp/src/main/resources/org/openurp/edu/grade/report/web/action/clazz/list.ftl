...
[#include "taskListTable.ftl"/]
  ...
  <script>
    ...
    function exportData(){
       ...
       bg.form.addInput(form,"keys","no,course.code,course.name,courseType.name,teachDepart.name,name,enrollment.actual,course.credits");
   ...
   function printStatReport(form,kind){
     ...
     function printExamReport(form){
        form.target="_blank";
        bg.form.submitId(form,"clazz.id",true,"${b.url('!reportForExam')}");
        form.target="_self";
     }
   //打印教学班成绩
   function printTeachClassGrade(form){
        form.target="_blank";
        bg.form.submitId(form,"clazz.id",true,"${b.url('!report')}");
        form.target="_self";
   }
  </script>
[@b.foot/]
