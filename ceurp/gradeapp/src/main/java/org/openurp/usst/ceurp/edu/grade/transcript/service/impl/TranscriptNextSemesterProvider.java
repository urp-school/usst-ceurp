...
public class TranscriptNextSemesterProvider implements TranscriptDataProvider {
  ...
  @Override
  public <T> Map<Student, T> getDatas(List<Student> stds, Map<String, String> options) {
    Map semestersMap = CollectUtils.newHashMap();

    List<Semester> semesters = entityDao.search(OqlBuilder.from(Semester.class, "semester").orderBy(Order.parse("semester.beginOn")));
  ...
  @Override
  public <T> T getData(Student std, Map<String, String> options) {
    Map<Student, T> mapData = getDatas(null, options);
    return mapData.isEmpty() ? null : mapData.get(0);
  }
